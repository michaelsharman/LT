 ...
 * @param {string=} options.heading The heading for the error dialog. Defaults to "Required tag{s} missing".
 * @param {number=} options.headingLevel The heading level for the error dialog. Defaults to `3`.
 * @param {string=} options.messageStart The start of the error message. Defaults to "Please add the following tag{s} before publishing:".
 * @param {string=} options.messageEnd The end of the error message. Defaults to "".
 * @param {array=} options.requiredTags The list of required tags. Defaults to [].
 *
 * @example
 * const options =  {
...
function createErrorDialog() {
    ...
    if (!elAuthorApp) {
        LT.utils.logger.error('No Author API element found');
        return;
    }
...
function renderDialog(missingTags, saveOnValid = false) {
    ...
    const tags = LT.authorApp().renderComponent('tags', elTagsUI);
    ...
    elSaveTagsBtn.onclick = () => {
        ...
        LT.authorApp().setItemTags(combinedTags);
...
function runValidation(saveOnValid = false) {
    const validItemTags = validateItemTags(LT.authorApp().getItemTags());
