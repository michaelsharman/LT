...
function E(e) {
  const r = (i) => {
    ...
    const v = (t) => {
      ...
      const p = c.getBoundingClientRect().width, d = (h + l) * 100 / p;
      ...
      if ((t.key === " " || t.key === "Spacebar") && (t.preventDefault(), s = !s, i.setAttribute("aria-pressed", String(s))), !s)
        return;
      const l = 2, p = c.getBoundingClientRect().width, d = o.getBoundingClientRect().width * 100 / p;
      if (t.key === "ArrowLeft") {
        t.preventDefault();
        const f = Math.max(10, d - l);
        o.style.width = f + "%";
      }
      if (t.key === "ArrowRight") {
        t.preventDefault();
        const f = Math.min(90, d + l);
        o.style.width = f + "%";
      }
    };
