import { c as P, e as U } from "./app-VrnQQRdL.js";
...
function J(c) {
  ...
  const h = `<div class="magnifier" style="display: none;position: fixed;overflow: hidden;background-color: white;border: 1px solid #555;border-radius: 4px;z-index:10000;">
  ...
  function q() {
    u && (window.clearTimeout(_), _ = window.setTimeout(B, 100));
  }
  function A() {
    u && q();
  }
  ...
  function $(e, t) {
    ...
    function V(s) {
      ...
      if (a && t.exclude.indexOf(a.tagName.toUpperCase()) == -1 && (!f || t.exclude.indexOf(f.tagName.toUpperCase()) == -1)) {
        n = e;
        const w = s.pageX || s.touches[0].pageX, O = s.pageY || s.touches[0].pageY;
        k = n.getBoundingClientRect().left - n.offsetLeft, H = n.getBoundingClientRect().top - n.offsetTop, g = w - (n.getBoundingClientRect().left + document.body.scrollLeft), C = O - (n.getBoundingClientRect().top + document.body.scrollTop), s.preventDefault();
      }
    }
    function Y(s) {
      if (n !== null) {
        const a = s.pageX || s.touches[0].pageX, f = s.pageY || s.touches[0].pageY, w = a - g - k - document.body.scrollLeft, O = f - C - H - document.body.scrollTop;
        v(n, w, O), t.ondrag && t.ondrag.call(s);
      }
    }
    function j() {
      n !== null && (n = null);
