import { u as g, n as v, c as s, o as i, p as w, r as k, v as o, w as b, x as c } from "./app-C2_EjRb0.js";
  ...
  s().on("item:load", () => {
    k().forEach((a) => {
      ...
      ["audio", "video"].includes(d) && (u.on("recording:started", () => {
        ...
        const l = p[a] || 0, { revision: f, value: y } = b(a);
...
function x() {
  setTimeout(() => {
    const r = document.querySelectorAll(".lrn-assess-dialogs > .lrn-dialog-default");
    let n = "";
    const p = Array.from(r).filter((a) => a.style.display === "block").map((a) => ({
      id: a.id,
      class: a.className
    }));
    return p?.id ? n = p.id.replace(/\d+/g, "") : p?.class.includes("review-screen") && (n = "review-screen"), n;
  }, 500);
}
function e() {
  return Date.now();
