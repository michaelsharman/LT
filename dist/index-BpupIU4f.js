import { e as P } from "./app-DV5bcV_K.js";
...
function Z(c) {
  g._initialised ? U.debug("Magnifier already initialised, ignoring run();", W) : (c || (c = {
    zoom: 4,
    shape: "square",
    width: 350,
    height: 350
  }), g.magnifier = new F(c), g._initialised = !0);
}
function G(c = "lrn__magnifier") {
  document.querySelectorAll(`.${c}`).forEach((h) => {
    h.addEventListener("click", () => {
      g.magnifier.toggle();
    });
  }), app.appInstance().on("item:load", J());
}
function Q() {
  g.magnifier.toggle();
}
  ...
  function M() {
    ...
    if (n.length > 0 && n.length === d.length)
      for (let p = 0; p < n.length; p++)
        d[p].getContext("2d").drawImage(n[p], 0, 0);
  ...
  function $(e, t) {
    ...
    function V(s) {
      ...
      if (a && t.exclude.indexOf(a.tagName.toUpperCase()) == -1 && (!f || t.exclude.indexOf(f.tagName.toUpperCase()) == -1)) {
        ...
        _ = n.getBoundingClientRect().left - n.offsetLeft, H = n.getBoundingClientRect().top - n.offsetTop, p = w - (n.getBoundingClientRect().left + document.body.scrollLeft), C = k - (n.getBoundingClientRect().top + document.body.scrollTop), s.preventDefault();
      }
    }
    function Y(s) {
      if (n !== null) {
        const a = s.pageX || s.touches[0].pageX, f = s.pageY || s.touches[0].pageY, w = a - p - _ - document.body.scrollLeft, k = f - C - H - document.body.scrollTop;
...
function J() {
  const o = P().querySelectorAll("img");
  o && o.forEach((h) => {
    h.addEventListener("click", (r) => {
      g.magnifier.isVisible() || g.magnifier.show(r);
    });
  });
}
