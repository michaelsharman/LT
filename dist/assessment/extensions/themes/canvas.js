...
function f() {
  n.elements.apiWrapper = document.querySelector(".lrn-assess"), n.elements.items = document.querySelectorAll(".inline-item");
}
function u() {
  n.elements.items.forEach((r) => {
    const o = r.getAttribute("data-reference");
    r.querySelector(".lrn-assess-content").insertAdjacentHTML("afterbegin", t(o));
  }), document.querySelectorAll(".item-flag").forEach((r) => {
    r.addEventListener("click", b);
  });
  function t(r) {
    return `<button type="button" class="item-flag" aria-label="Flag item" data-reference="${r}">
...
function w() {
  n.elements.items.forEach((e) => {
    if (!e.querySelector(".row")) {
      const r = e.querySelector(".numbered-question"), o = e.querySelector(".question-number");
      r.classList.add("position-relative"), o.classList.add("extra-left-position");
    }
  });
}
function x() {
  const e = document.createElement("style"), t = m;
  e.setAttribute("data-style", "LT Theme Canvas"), e.textContent = t, document.head.append(e), n.renderedCss = !0;
}
const q = p("canvas", c);
export {
  q as canvas
