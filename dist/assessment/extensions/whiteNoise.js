    ...
    (function() {
      ...
      l.prototype = {
        ...
        play: function(e, t) {
          ...
          var h = Math.max(0, a._seek > 0 ? a._seek : n._sprite[e][0] / 1e3), b = Math.max(0, (n._sprite[e][0] + n._sprite[e][1]) / 1e3 - h), T = b * 1e3 / Math.abs(a._rate), k = n._sprite[e][0] / 1e3, H = (n._sprite[e][0] + n._sprite[e][1]) / 1e3;
          a._sprite = e, a._ended = !1;
          var L = function() {
            a._paused = !1, a._seek = h, a._start = k, a._stop = H, a._loop = !!(a._loop || n._sprite[e][2]);
          };
          if (h >= H) {
            n._ended(a);
            return;
          ...
          if (n._webAudio) {
            var O = function() {
              n._playLock = !1, L(), n._refreshBuffer(a);
              var S = a._muted || n._muted ? 0 : a._volume;
              y.gain.setValueAtTime(S, r.ctx.currentTime), a._playStart = r.ctx.currentTime, typeof y.bufferSource.start > "u" ? a._loop ? y.bufferSource.noteGrainOn(0, h, 86400) : y.bufferSource.noteGrainOn(0, h, b) : a._loop ? y.bufferSource.start(0, h, 86400) : y.bufferSource.start(0, h, b), T !== 1 / 0 && (n._endTimers[a._id] = setTimeout(n._ended.bind(n, a), T)), t || setTimeout(function() {
                n._emit("play", a._id), n._loadQueue();
              }, 0);
            };
            r.state === "running" && r.ctx.state !== "interrupted" ? O() : (n._playLock = !0, n.once("resume", O), n._clearTimer(a._id));
          } else {
            var B = function() {
              y.currentTime = h, y.muted = a._muted || n._muted || r._muted || y.muted, y.volume = a._volume * r.volume(), y.playbackRate = a._rate;
              try {
                var S = y.play();
                if (S && typeof Promise < "u" && (S instanceof Promise || typeof S.then == "function") ? (n._playLock = !0, L(), S.then(function() {
                ...
                })) : t || (n._playLock = !1, L(), n._emit("play", a._id)), y.playbackRate = a._rate, y.paused) {
            ...
            var G = window && window.ejecta || !y.readyState && r._navigator.isCocoonJS;
            if (y.readyState >= 3 || G)
              B();
            else {
              n._playLock = !0, n._state = "loading";
              var C = function() {
                n._state = "loaded", B(), y.removeEventListener(r._canPlayEvent, C, !1);
              };
              y.addEventListener(r._canPlayEvent, C, !1), n._clearTimer(a._id);
            }
          }
          return a._id;
        },
        ...
        _startFadeInterval: function(e, t, n, o, s, u) {
          var a = this, c = t, h = n - t, b = Math.abs(h / 0.01), T = Math.max(4, b > 0 ? o / b : o), k = Date.now();
          e._fadeTo = n, e._interval = setInterval(function() {
            var H = (Date.now() - k) / o;
            k = Date.now(), c += h * H, c = Math.round(c * 100) / 100, h < 0 ? c = Math.max(n, c) : c = Math.min(n, c), a._webAudio ? e._volume = c : a.volume(c, e._id, !0), u && (a._volume = c), (n < t && c <= n || n > t && c >= n) && (clearInterval(e._interval), e._interval = null, e._fadeTo = null, a.volume(n, e._id), a._emit("fade", e._id));
          }, T);
        },
        /**
...
function W(g = {}) {
  const { id: m = null, shadowRoot: r = document } = g;
  v.elementId = m, v.queryRoot = r;
  try {
    x.assessApp().on("button:btn-whitenoise:clicked", () => {
      D();
    });
  } catch (l) {
    l instanceof TypeError || x.utils.logger.error("Error with white noise player:", l);
  }
}
function D() {
  const g = Y();
  if (v.elementId && !v.shadowRoot) {
    ...
    if (m)
      ...
      x.utils.logger.error(`Element id '${v.elementId}' not found, could not render player.`);
      return;
    }
    ...
    if (m)
      ...
      x.utils.logger.error(`Shadow root element id '${v.elementId}' not found, could not render player.`);
      return;
    }
  } else
    x.dialog({
  ...
  try {
    x.assessApp().on("button:dialog_btn_whitenoise_player:clicked", () => {
      x.hideDialog();
    });
  } catch (m) {
    m instanceof TypeError || x.utils.logger.error("Error with white noise player:", m);
  }
}
function z(g) {
  const m = g.getAttribute("data-lt-sound"), r = v.queryRoot.querySelector(`[data-lt-sound="${m}"]`);
  v.player.sound && E(v.player.sound), r.classList.contains("lt__sound-active") ? E(m) : (v.player.instances[m] || N(m), v.player.sound = m, Q(m)), q(m);
}
function N(g) {
  v.player.instances[g] = new V.Howl({
    src: [v.playlist[g]],
    html5: !0,
    loop: !0
  });
...
function Q(g) {
  v.player.instances[g].play();
}
function E(g) {
  v.player.instances[g].stop();
}
function F() {
  const g = v.queryRoot.querySelector("#ld-volume"), m = v.queryRoot.querySelector("#ld-volume-value"), r = g.value;
  v.player.volume = r, V.Howler.volume(r), m.innerHTML = r * 100;
}
function q(g) {
  v.queryRoot.querySelectorAll(".lt__controls-sound").forEach((r) => {
    r.getAttribute("data-lt-sound") === g && !r.classList.contains("lt__sound-active") ? (r.classList.add("lt__sound-active"), r.focus(), r.setAttribute("aria-pressed", "true")) : (r.classList.remove("lt__sound-active"), r.setAttribute("aria-pressed", "false"));
  });
}
function Y() {
...
function Z() {
  const m = `
        ...
        ${v.queryRoot !== document ? ":host" : ":root"} {
            --lt-wn-border: #888888;
    ...
    `;
  if (v.queryRoot === document)
    return m;
  {
    const r = document.createElement("style");
    return r.setAttribute("data-style", "LT White Noise"), r.textContent = m, v.queryRoot.appendChild(r), "";
  }
}
const j = $("whiteNoise", W, {
  getStyles: Z,
  launchPlayer: D
});
export {
