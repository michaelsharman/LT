...
function E(e) {
  const r = (i) => {
    ...
    let s = !1, h = 0, u = 0;
    const v = (t) => {
      t instanceof MouseEvent ? u = t.clientX : u = t.targetTouches[0].clientX, h = o.getBoundingClientRect().width, s = !0, i.setAttribute("aria-pressed", "true"), document.addEventListener("mousemove", m), document.addEventListener("mouseup", p), document.addEventListener("touchmove", m), document.addEventListener("touchend", p);
    }, m = (t) => {
      if (!s)
        return;
      let l;
      t instanceof MouseEvent ? l = t.clientX - u : l = t.targetTouches[0].clientX - u;
      const g = c.getBoundingClientRect().width, d = (h + l) * 100 / g;
...
.lt__resizer[aria-pressed="true"] .lt__resizer-tab {
