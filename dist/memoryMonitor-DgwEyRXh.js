import { a as E } from "./extensionsFactory-CJF5B414.js";
...
function k(a, { limit: t = 50 } = {}) {
  if (!a || !w || !w.length)
    return;
  const i = 10 ** 3, s = (o) => Number.isFinite(o) ? Math.round((o + Number.EPSILON) * i) / i : o, r = (o, p) => o.reduce((l, h) => l + (Number.isFinite(h[p]) ? h[p] : 0), 0), u = /* @__PURE__ */ new Map();
  for (const o of w)
  ...
  for (const [o, p] of u) {
    ...
    for (const h of p)
      h.phase === "import" ? l.importMs += h.ms : h.phase === "css" ? l.cssMs += h.ms : h.phase === "run" && (l.runMs += h.ms);
    l.totalMs = l.importMs + l.cssMs + l.runMs, m.push(l);
  }
  m.sort((o, p) => p.totalMs - o.totalMs);
...
async function N(a, t, e, { mode: i = "sequential", collectCSS: s = !0, dedupeCSS: r = !0, mountId: u = "lt-extensions", perf: m = !1, perfLimit: f = 50, security: o = {}, request: p = {} } = {}) {
  const l = d();
  E(a, o, p);
  const h = (t || []).map(I);
  a.extensions ||= {};
...
${y}`), r && T.add(n));
  ...
  if (i === "parallel") {
    const n = h.map((c) => S(c).catch((M) => b.error(`[LT] Failed to init extension "${c.id}"`, M)));
...
class O {
  ...
  #t(t) {
    if (!t.length)
      return null;
    const e = t.map((p) => p.usedMB), i = Math.min(...e), s = Math.max(...e), r = +(e.reduce((p, l) => p + l, 0) / e.length).toFixed(2), u = t[0].ts, m = t.map((p) => (p.ts - u) / 6e4), { slope: f, r2: o } = this.#e(m, e);
    return {
      count: t.length,
      minMB: i,
      maxMB: s,
  ...
  #e(t, e) {
    ...
    const s = t.reduce((n, c) => n + c, 0), r = e.reduce((n, c) => n + c, 0), u = t.reduce((n, c, M) => n + c * e[M], 0), m = t.reduce((n, c) => n + c * c, 0), f = s / i, o = r / i, p = u - i * f * o, l = m - i * f * f, h = l ? p / l : 0, g = o - h * f, T = e.reduce((n, c) => n + (c - o) ** 2, 0), B = e.reduce((n, c, M) => n + (c - (h * t[M] + g)) ** 2, 0), S = T ? 1 - B / T : 0;
    return { slope: h, intercept: g, r2: S };
  }
}
export {
  P as E,
