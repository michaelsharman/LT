import { c as x, l as y } from "./app-C501rTFe.js";
const d = {
  renderedCss: !1,
  resize: {
    triggered: !1
...
function _() {
  d.renderedCss || k(), x().on("item:load", () => {
    v();
...
function E(e) {
  const r = (i) => {
    ...
    let s = !1, h = 0, u = 0;
    const b = (t) => {
      t instanceof MouseEvent ? u = t.clientX : u = t.targetTouches[0].clientX, h = o.getBoundingClientRect().width, s = !0, i.setAttribute("aria-pressed", "true"), document.addEventListener("mousemove", m), document.addEventListener("mouseup", g), document.addEventListener("touchmove", m), document.addEventListener("touchend", g);
    }, m = (t) => {
      if (!s)
        return;
      let l;
      t instanceof MouseEvent ? l = t.clientX - u : l = t.targetTouches[0].clientX - u;
      const p = c.getBoundingClientRect().width, a = (h + l) * 100 / p;
      a >= 10 && a <= 90 && (o.style.width = a + "%");
    }, g = () => {
      s = !1, i.setAttribute("aria-pressed", "false"), document.removeEventListener("mousemove", m), document.removeEventListener("mouseup", g), document.removeEventListener("touchmove", m), document.removeEventListener("touchend", g);
    }, w = (t) => {
      if ((t.key === " " || t.key === "Spacebar") && (t.preventDefault(), s = !s, i.setAttribute("aria-pressed", String(s))), !s)
        return;
      const l = 2, p = c.getBoundingClientRect().width, a = o.getBoundingClientRect().width * 100 / p;
      if (t.key === "ArrowLeft") {
        t.preventDefault();
        const f = Math.max(10, a - l);
        o.style.width = f + "%";
      }
      if (t.key === "ArrowRight") {
        t.preventDefault();
        const f = Math.min(90, a + l);
        o.style.width = f + "%";
      }
    };
    i.addEventListener("mousedown", b), i.addEventListener("touchstart", b), i.addEventListener("keydown", w);
